...
> **基于 Open-AutoGLM 深度重构，补齐工程化的最后一公里。致力于提供稳定、可用的 Android Agent 基础设施。**
...
- **实时持久化** - 任务状态每步自动保存，服务重启不丢失进度
- **全链路调试** - 透明展示 LLM 思考过程、Prompt 上下文和原始响应
- **自愈机制** - 视频流/WebSocket 断连自动重连，防止任务挂起
- **智能防抖** - 截图前自动检测 UI 稳定性，防止获取模糊或黑屏图像
...
**动态配置**：
支持在 Web 界面实时切换模型提供商（OpenAI/Gemini/Qwen）和 Base URL，配置自动持久化，无需频繁修改环境变量。
