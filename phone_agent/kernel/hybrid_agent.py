...
class HybridAgent:
    ...
    def __init__(
        ...
        # 性能统计
        self._stats = {"xml_success": 0, "xml_failure": 0, "vision_success": 0, "vision_failure": 0}
        ...
        try:
            ...
            if not self._vision_agent:
                ...
                self._vision_agent = PhoneAgent(
                    model_config=self.model_config, agent_config=agent_config
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vision": {
                ...
                "total": total_vision,
                "success_rate": (
                    self._stats["vision_success"] / total_vision if total_vision > 0 else 0
                ),
            },
        }
...
if __name__ == "__main__":
    ...
    agent = HybridAgent(
        model_config=model_config,
        config=HybridConfig(
            mode=ExecutionMode.AUTO, device_id="localhost:6100", verbose=True  # 自动选择
        ),
    )
    ...
    print("性能统计:")
    print(
        f"  XML: {stats['xml']['success']}/{stats['xml']['total']} ({stats['xml']['success_rate']:.1%})"
    )
    print(
        f"  Vision: {stats['vision']['success']}/{stats['vision']['total']} ({stats['vision']['success_rate']:.1%})"
    )
