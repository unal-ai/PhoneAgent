...
class XMLKernelAgent:
    ...
    def __init__(
        ...
        if self.config.verbose:
            logger.warning(" XML Kernel is in BETA. For production, use Vision Kernel.")
    ...
    def run(self, goal: str) -> Dict[str, Any]:
        ...
        """
        if self.config.verbose:
            logger.info("XML Kernel Agent 启动")
        ...
        for step in range(self.config.max_steps):
            ...
            try:
                ...
                if not elements:
                    logger.warning(f"未找到UI元素 (步骤{self._step_count}/{self.config.max_steps})")
        ...
        # 达到最大步骤数
        logger.warning(f"达到最大步骤数 ({self.config.max_steps})，任务未完成")
    ...
    def _get_llm_decision(
        ...
        if len(self._context) > (
            self.config.max_context_turns * 2 + 1
            ...
            # 保留最近N-1轮（因为已经保留了第一轮）
            recent_messages = self._context[
                -(self.config.max_context_turns * 2 - 2) :
            ]  # 保留最近N-1轮
...
请继续执行目标，决定下一步操作。"""
        ...
        try:
            ...
            response = self.model_client.request_json(
                messages=self._context, temperature=0.1  # 降低随机性，提高稳定性
            )
            ...
            if not response.raw_content or not response.raw_content.strip():
                ...
                return {"action": "wait", "reason": "LLM返回空响应，等待重试"}
            ...
            try:
                ...
                try:
                    ...
                    return {"action": "wait", "reason": f"JSON解析失败: {str(je)}"}
            ...
            if isinstance(decision, list):
                logger.warning(f"decision是list而非dict: {decision}")
                # 尝试提取第一个元素
                if decision and isinstance(decision[0], dict):
                    decision = decision[0]
                else:
                    return {"action": "wait", "reason": "LLM返回了数组格式，无法解析"}

            if not isinstance(decision, dict):
                logger.error(f"decision格式错误: {type(decision)}")
                return {"action": "wait", "reason": f"decision格式错误: {type(decision)}"}

            # 验证必需字段
            if "action" not in decision:
                logger.warning(f"decision缺少action字段: {decision}")
                return {"action": "wait", "reason": "LLM响应缺少action字段"}

            # 记录助手回复
            self._context.append({"role": "assistant", "content": response.raw_content})
            ...
            # 降级：返回等待动作
            return {"action": "wait", "reason": f"LLM调用失败: {str(e)}"}
...
if __name__ == "__main__":
    ...
    # 配置模型
    model_config = ModelConfig(
        api_key="your-api-key", base_url="https://open.bigmodel.cn/api/paas/v4/", model_name="glm-4"
    )

    # 创建agent
    agent = XMLKernelAgent(
        model_config=model_config, config=XMLKernelConfig(device_id="localhost:6100", verbose=True)
    )
