...
config = Config()
setup_logging(
    log_level=config.LOG_LEVEL, log_file="phoneagent.log", enable_console=True, enable_file=True
)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(
            f"App config manager initialized: {stats['total']} apps ({stats['enabled']} enabled)"
        )
        ...
        while True:
            try:
                ...
                await ws_manager.broadcast(
                    {"type": "device_update", "data": device_pool.get_stats()}
                )

                # 广播任务状态
                await ws_manager.broadcast(
                    {"type": "task_update", "data": agent_service.get_stats()}
                )
...
def create_app() -> FastAPI:
    ...
    app.add_middleware(
        RequestLoggerMiddleware, exclude_paths=["/health", "/api/docs", "/api/redoc"]
    )
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host=config.SERVER_HOST, port=8000, log_level="info")
