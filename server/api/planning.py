...
class ModelConfig(BaseModel):
    ...
    kernel_mode: Optional[str] = Field(
        default="auto", description="执行内核模式: xml(快速), vision(兜底), auto(自动)"
    )
...
class GeneratePlanRequest(BaseModel):
    ...
    model_settings: Optional[ModelConfig] = Field(
        None, alias="model_config", description="模型配置（可选，留空使用环境变量）"
    )
...
class ExecutePlanRequest(BaseModel):
    ...
    use_smart_positioning: bool = Field(
        default=True, description="是否使用XML智能定位（提升准确率）"
    )
    ...
    try:
        ...
        if model_config_dict:
            ...
            if not model_name or not base_url or not api_key:
                ...
                logger.info("📝 部分配置来自环境变量")
            else:
                logger.info("🎯 使用用户指定配置")
        else:
            # 完全使用环境变量配置
            from server.utils.model_config_helper import get_model_config_from_env
        ...
        plan = await asyncio.to_thread(
            planner.generate_plan, request.instruction, include_screenshot=True
        )
        ...
        return {"success": True, "plan": plan.to_dict(), "message": "计划生成成功"}
...
async def execute_direct(request: ExecuteDirectRequest):
    ...
    try:
        ...
        if request.model_settings:
            model_config_dict = (
                request.model_settings if isinstance(request.model_settings, dict) else {}
            )
        else:
            model_config_dict = {}
        ...
        agent_service = get_agent_service()
        task_id = await agent_service.create_task(
            instruction=plan.instruction, device_id=request.device_id, model_config=None
        )
