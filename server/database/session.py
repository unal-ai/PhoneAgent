...
def _create_indexes(engine):
    ...
    indexes = [
        ...
        (
            "idx_device_active",
            "CREATE INDEX IF NOT EXISTS idx_device_active ON devices(last_active DESC);",
        ),
        ("idx_device_busy", "CREATE INDEX IF NOT EXISTS idx_device_busy ON devices(is_busy);"),
        # 模型调用统计表索引
        (
            "idx_model_call_task",
            "CREATE INDEX IF NOT EXISTS idx_model_call_task ON model_calls(task_id);",
        ),
        (
            "idx_model_call_time",
            "CREATE INDEX IF NOT EXISTS idx_model_call_time ON model_calls(called_at DESC);",
        ),
        (
            "idx_model_call_provider",
            "CREATE INDEX IF NOT EXISTS idx_model_call_provider ON model_calls(provider, model_name);",
        ),
        (
            "idx_model_call_kernel",
            "CREATE INDEX IF NOT EXISTS idx_model_call_kernel ON model_calls(kernel_mode);",
        ),
    ]
        ...
        for index_name, sql in indexes:
            try:
                ...
                if "no such column" in str(e):
                    logger.warning(
                        f" Index {index_name} skipped: column not found (database schema may be outdated)"
                    )
                else:
    ...
    if failed_count == 0:
        ...
        logger.warning(
            f" Database indexes partially created ({created_count}/{len(indexes)}, {failed_count} failed)"
        )
