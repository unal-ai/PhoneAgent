...
class ScrcpySession:
    ...
    def start(self, bitrate: int = 4_000_000, max_size: int = 1280, framerate: int = 30):
        ...
        try:
            ...
            self._read_thread = threading.Thread(
                target=self._read_nal_units_from_socket, daemon=True
            )
    ...
    def _setup_port_forward(self, adb_address: str):
        ...
        try:
            ...
            subprocess.run(
                cmd_remove, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=2
            )
        except Exception:
            pass
    ...
    def _start_scrcpy_server(self, adb_address: str, bitrate: int, max_size: int, framerate: int):
        ...
        if check_result.returncode != 0:
            raise RuntimeError(
                "scrcpy-server not found on device. Please install it first. "
                "Expected at /data/local/tmp/scrcpy-server"
            )
